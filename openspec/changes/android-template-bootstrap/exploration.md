...
| Tests | No test runner | Baseline test proof | One local JUnit test and one instrumented AndroidX/Espresso smoke test; no coverage threshold | Proves both source sets and runner wiring without product assumptions. |
...
| Public safety | Privacy and security rules | Neutral identifiers and non-secret project metadata | Use placeholders such as `com.example.template` and `Template App`; require adopters to replace both before publication | Prevents template identity from becoming a shipping package ID. |
