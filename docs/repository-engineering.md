...
| Android build | Debug build | `./gradlew assembleDebug` | `<UNRESOLVED_OWNER>` | Confirms the neutral debug application compiles with the approved toolchain. | Run the command and record the result. | Accepted template baseline |
| Android lint | Android Lint | `./gradlew lint` | `<UNRESOLVED_OWNER>` | Android Lint is the built-in v1 static-analysis gate; no additional static-analysis tool is required. | Run the command and record the result. | Accepted template baseline |
| Android JVM tests | JVM test task | `./gradlew testDebugUnitTest` | `<UNRESOLVED_OWNER>` | Retains a canonical JVM quality command without inventing a tautological starter test when no meaningful pure-JVM behavior exists. | Run the command and record the result. | Accepted template baseline |
| Android instrumentation | Connected smoke test | `./gradlew connectedDebugAndroidTest` | `<UNRESOLVED_OWNER>` | Instrumented validation remains a separate gate because it requires a configured device or emulator. | Run when a device/emulator is available; record an unavailable prerequisite separately. | Accepted template baseline |
| Android static tooling | Formatter/static-analysis policy | No ktlint, Spotless, Detekt, or Kover in v1 | `<UNRESOLVED_OWNER>` | Avoid overlapping maintenance until product logic demonstrates a need. | Android build, lint, and JVM test rows. | Deferred by design |
